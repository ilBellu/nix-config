...
in
  {
    ...
    config = mkIf cfg.enable {
      ...
      boot = {
        ...
        blacklistedKernelModules = if (cfg.cpu == "intel") then mkDefault ["i915"] else mkDefault ["amdgpu"];
        kernelParams = if (cfg.cpu == "intel") then mkDefault ["i915.modeset=0"] else mkDefault ["radeon.modeset=0"];
